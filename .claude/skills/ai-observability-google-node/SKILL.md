---
name: ai-observability-google-node
description: PostHog AI Observability integration for Google Gemini (Node)
metadata:
  author: PostHog
  version: 1.52.0
---

# PostHog AI Observability for Google Gemini (Node)

Wire up PostHog's AI Observability so calls made through Google Gemini (Node) land in LLM Analytics as a full **session → trace → span → generation** tree — not just isolated `$ai_generation` events.
